...
public class SelectedSupplier implements Serializable {
	...
	public PaginationHelper getPagination() {
		...
		@Override public DataModel<PurchaseOrder>createPageDataModel(){
			try{
				return new ListDataModel<PurchaseOrder>(purchaseOrderService.findPurchaseOrderBySupplier(supplier).subList(getPageFirstItem(),getPageFirstItem() + getPageSize()));
			} catch (Exception e) {
				return new ListDataModel<PurchaseOrder>(
						purchaseOrderService.findPurchaseOrderBySupplier(supplier).subList(getPageFirstItem(),getItemsCount()));
		}
		}
		};
		return pagination;
